...
int main(int argc, char* argv[])
{
    ...
    const int gn_iters = 100; // maximum number of Gauss Newton iterations
    ...
    // Constants
    const Matrix3f I = gn_eps*Matrix3f::Identity();
    ...
    for (ii = 0; ii < gn_iters; ++ii)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            Matrix<float,Np,1> errp = computeRadioErrorAll<Np>(pts2p , pts2);
            Matrix<float,Np,1> errm = computeRadioErrorAll<Np>(pts2m , pts2);
            J.col(i) = (errp - errm)/(2.0*gn_eps);
        }
        for (int i = 0; i < 3; ++i)
        {
            ...
            Matrix<float,Np,1> errp = computeRadioErrorAll<Np>(pts2p , pts2);
            Matrix<float,Np,1> errm = computeRadioErrorAll<Np>(pts2m , pts2);
            J.col(i+3) = (errp - errm)/(2.0*gn_eps);
        }

        // Computer current error
        vector<Point> pts2_hat = predictPointLocs(pts1, K, K_inv, q_hat, t_hat);
        Matrix<float,Np,1> err = computeRadioErrorAll<Np>(pts2_hat, pts2);
